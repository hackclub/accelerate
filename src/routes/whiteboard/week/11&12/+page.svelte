<script>
    ...
    onMount(() => {
        const targetDate = new Date('2026-01-30T00:00:00-05:00'); 
...
<div>
    ...
    <iframe 
        src="https://uw8gksso8c00cgw0k4448ws0.a.selfhosted.hackclub.com" 
        class="absolute w-3/4 h-[75vh] overflow-hidden scale-80 z-40 -right-80 translate-y-25 drop-shadow-6xl" 
        scrolling="no"
        title="Simulation"
    ></iframe>
    ...
    <h1 class={expanded ? "hidden" : "absolute top-30 left-15 text-4xl md:text-5xl font-bold z-50 text-neutral-700 drop-shadow-lg"}>
        Week 11&12: Finite Element Analysis
    </h1>
    ...
    <p class = "absolute bottom-42 left-100 text-xl font-pangolin">
        ...
        This week you're going to build a FEA Solver (Finite Element Analysis)! <br>
        Your challenge is a bit different.<br> Find an interesting design that you want to model, (wind turbine, crane etc.)<br> and ship an FEA Solver of it as either a web-page,
        ...
        <span class="relative left-10 text-2xl font-pangolin">You modeled the Stress Strain on a wind turbine blade?</span><br>
        <span class="relative left-10 text-2xl font-pangolin">You modelled a crane picking up a load?</span><br>
        <span class="relative left-10 text-2xl font-pangolin">There were multiple materials involved?</span><br>
        <span class="relative left-10 text-2xl font-pangolin">The materials could deform?</span><br>
        <span class="relative left-10 text-2xl font-pangolin">The materials could break?</span>
    </p>
    <button class="absolute bottom-10 left-120 hover:cursor-pointer" onclick={() => window.location.replace("/whiteboard/week/11%2612/submit")} onmouseenter={() => (hoverSubmit = true)} onmouseleave={() => (hoverSubmit = false)}>
