<script>
    ...
    onMount(() => {
        const targetDate = new Date('2025-12-30T00:00:00-05:00'); // December 30, 2025, 12:00 AM EST
...
<div>
    ...
    <iframe 
        src="https://moks8sco4wwgss0cw0cg0s4s.a.selfhosted.hackclub.com" 
        class="absolute w-3/4 h-[75vh] overflow-hidden scale-100 z-40 -right-100 -translate-y-25 drop-shadow-6xl" 
        scrolling="no"
        title="Simulation"
    ></iframe><div class={expanded
        ? "fixed inset-0 z-50 w-screen h-screen"
        : "absolute left-10 bottom-15 w-full max-w-[20rem] h-[55vh] z-40 rounded-lg"}>
        <iframe
            src="/Weeks_7_8__Electromagnetic_Fields.pdf"
    ...
    <h1 class={expanded ? "hidden" : "absolute top-30 left-15 text-4xl md:text-5xl font-bold z-50 text-neutral-700 drop-shadow-lg"}>
        Week 7&8: Electrostatics & Electromagnetic Waves
    </h1>
    ...
    <p class = "absolute bottom-42 left-100 text-xl font-pangolin">
        ...
        This week you're challenged to build your own Electromagnetic/Electrostatic simulation! <br>
        ...
        <span class="relative left-10 text-2xl font-pangolin">You simulated particle accelerators?</span><br>
        <span class="relative left-10 text-2xl font-pangolin">You simulated Electromagnetic refraction and reflection?</span><br>
        <span class="relative left-10 text-2xl font-pangolin">You simulated a electric generator?</span><br>
        <span class="relative left-10 text-2xl font-pangolin">There was a variable background field?</span><br>
        <span class="relative left-10 text-2xl font-pangolin">The other forces were acting on statics in an electric field?</span>
    </p>
    <button class="absolute bottom-10 left-120 hover:cursor-pointer" onclick={() => window.location.replace("/whiteboard/week/7%268/submit")} onmouseenter={() => (hoverSubmit = true)} onmouseleave={() => (hoverSubmit = false)}>
